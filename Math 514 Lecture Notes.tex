...
\begin{equation}
	E(x) = \frac{f^{(n+1)}(\xi)}{n!} \prod_{k=0}^n (x-x_k)
\end{equation}
...
\begin{equation}
	0 = G^{(n+1)}(\xi) = E^{(n+1)}(\xi) - \frac{n!}{\prod_{k=0}^n (t-x_k)}E(t)
\end{equation}
Then notice that 
\begin{equation}
	E^{(n+1)}(\xi) = f^{(n+1)}(\xi)
...
so that
\begin{equation}
	f^{(n+1)}(\xi) - \frac{(n!}{\prod_{k=0}^n (t-x_k)}E(t) = 0
\end{equation}
...
Observations:
\begin{enumerate}
	\item If $f(x) \in \mathbb{P}_{N-1}(x)$, then $f^{(N)}(\xi) = 0$, so that $E(t) = 0$ for all $t$.
\end{enumerate}
